...
// Enhanced hover scale factor for dual-holder wallets to make them more prominent
const dualHolderHoverScaleFactor = 2.0; // Bigger scale increase for wallets with both tokens
...
if (sharedPoints.length > 0 && fartcoinPoints.length > 0 && goatTokenPoints.length > 0) {
  ...
  // Create spiral disc visualization with 2000 wallets
...
function animate() {
  ...
  // Add spiral disc wallet nodes for hover detection
  ...
  if (intersects.length > 0) {
    ...
    if (object.userData && object.userData.walletData) {
      ...
      if (hoveredObject !== object) {
        ...
        // Scale up and brighten the hovered object with special handling for dual-holder wallets
        let newScale;
        
        // Check if this is a dual-holder wallet (has both Fartcoin and Goat tokens)
        const isDualHolder = object.userData.walletData.fartAmount > 0 && object.userData.walletData.goatAmount > 0;
        
        if (isDualHolder) {
          // Use enhanced scale factor for dual-holder wallets
          newScale = object.userData.originalScale * dualHolderHoverScaleFactor;
          console.log(`Dual-holder wallet! Using enhanced scale: ${newScale}`);
        } else {
          // Use standard scale factor for regular wallets
          newScale = object.userData.originalScale * 3; // Increased from 1.5 to 3 for better visibility
        }
        ...
        // Brighten the color, with special white glow for dual-holders
        if (object.material) {
          ...
          if (isDualHolder) {
            // For dual-holders, ensure they glow white with maximum brightness
            brighterColor = new THREE.Color(1.0, 1.0, 1.0);
            console.log('Using WHITE glow for dual-holder wallet!');
          } else {
            // For single-token wallets, just brighten their original color
            brighterColor = new THREE.Color(
